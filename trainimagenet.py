...
parser.add_argument('--data', default='/data/ILSVRC2012/', metavar='DIR', help='path to dataset')
...
def main():
    ...
    # teacher model
    model_teacher = torchvision.models.resnet34(pretrained=True)
    ...
    criterion_kd = utils_loss.DistillationLoss().cuda() ## changed here
    ...
    optimizer = torch.optim.Adam([{'params': other_parameters},
                                 {'params': weight_parameters}], lr=2.5e-3, weight_decay=0)

    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda step : (1.0-step/args.epochs), last_epoch=-1)
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [70, 90, 100, 110], gamma=0.1) # original
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [30, 60, 90, 110], gamma=0.1)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.epochs, eta_min = 0, last_epoch=-1)

    start_epoch = 0
    best_top1_acc= 0

    checkpoint_tar = os.path.join(args.save, 'checkpoint-1.pth.tar')
...
def train(epoch, train_loader, model, model_teacher, criterion, optimizer, scheduler, meta_optim=None, meta_scheduler=None, criterion_meta=None ):
    ...
    # print('meta corr:', param[-1][1].data)
    ...
    scheduler.step()
    if use_meta != 'NoMeta' and meta_optim is not None:
        meta_scheduler.step()
